...
class NeuralNetwork:
    """Простая нейронная сеть для принятия решений птицы"""
...
class FlappyBirdAI:
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("Flappy Bird AI - Генетический Алгоритм")
    ...
    def draw_ui(self):
        ...
        instruction2 = self.small_font.render("R - Быстрое обучение", True, WHITE)
        ...
        if best_bird:
            pygame.draw.circle(self.screen, YELLOW, (int(best_bird.x), int(best_bird.y)), 18, 3)
    ...
    def run(self):
        ...
        while running:
            ...
            if not fast_mode or self.population.generation % 10 == 0:
                ...
                if fast_mode:
                    fast_text = self.small_font.render("БЫСТРОЕ ОБУЧЕНИЕ", True, YELLOW)
                    self.screen.blit(fast_text, (SCREEN_WIDTH//2 - 80, SCREEN_HEIGHT - 30))
                
                pygame.display.flip()
            
            # Контролируем FPS (в быстром режиме без ограничений)
            if not fast_mode:
                self.clock.tick(FPS)
...
if __name__ == "__main__":
    print("Добро пожаловать в Flappy Bird AI!")
    print("Наблюдайте, как птицы учатся играть с нуля!")
    print("\nУправление:")
    print("SPACE - Пауза/Продолжить")
    print("R - Быстрое обучение (ускоряет процесс)")
    print("ESC - Выход")
    print("\nИскусственный интеллект начинает обучение...")
